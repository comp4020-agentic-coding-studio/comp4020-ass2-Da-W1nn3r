...
- this is the move a splitter can't make: a T-junction crosses lane content
  across the merging belt's own left/right divide, feeding both of it into
  one lane on the other side
- combine it with a splitter (which relocates whole belts freely but never
  touches lane identity) and the two together can put any input's content on
  any output's lane — a splitter-only network never can
- merging onto a belt that's already full doesn't split evenly either: only
  the upstream lane gets through, the far lane just backs up behind it —
  worth testing directly rather than assuming it behaves like a splitter
...
- the standard 4-to-4 balancer is the textbook case for the first
  condition: a splitter-only graph model says 4 splitters are enough, and
  they are, for full load — but with only some inputs and outputs active,
  two of the four outputs turn out to be fed by only one splitter each,
  capping them at one belt's worth between them even though two full belts
  feed that splitter
...
- that formula counts splitters, not T-junctions — it answers "does every
  belt get an even share," which is a separate question from "does every
  lane," and a design only earns the name "balancer" once both are true
